...
package com.tommywebdesigns.apiassignment.model.data;
...
public class AnimeResult implements Parcelable {
    ...
    protected AnimeResult(Parcel in) {
        if (in.readByte() == 0) {
            malId = null;
        } else {
            malId = in.readInt();
    ...
    public static final Creator<AnimeResult> CREATOR = new Creator<AnimeResult>() {
        @Override
        public AnimeResult createFromParcel(Parcel in) {
            return new AnimeResult(in);
        }

        @Override
        public AnimeResult[] newArray(int size) {
